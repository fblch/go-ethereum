...
func (f *ForkChoice) ReorgNeeded(current *types.Header, extern *types.Header, dummy bool) (bool, error) {
	var (
	...
	if externNumber < localNumber {
		...
		if f.deterministic != nil && f.deterministic() {
			...
			if extern.GasUsed > current.GasUsed {
				reorg = true
			} else if extern.GasUsed == current.GasUsed {
