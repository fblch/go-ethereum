...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, isDeterministic func() bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	bc.forker = NewForkChoice(bc, shouldPreserve, isDeterministic)
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	updateHead := func(head *types.Block) bool {
		...
		if bc.CurrentHeader().Number.Cmp(head.Number()) >= 0 {
			...
			reorg, err := bc.forker.ReorgNeeded(bc.CurrentSnapBlock(), head.Header(), false)
			if err != nil {
...
func (bc *BlockChain) writeBlockAndSetHead(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool) (status WriteStatus, err error) {
	...
	reorg, err := bc.forker.ReorgNeeded(currentBlock, block.Header(), false)
	if err != nil {
		return NonStatTy, err
	}
	if reorg {
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals, setHead bool) (int, error) {
	...
	if bc.skipBlock(err, it) {
		...
		for block != nil && bc.skipBlock(err, it) {
			...
			reorg, err = bc.forker.ReorgNeeded(current, block.Header(), false)
			if err != nil {
				return it.index, err
			}
			if reorg {
...
func (bc *BlockChain) insertSideChain(block *types.Block, it *insertIterator) (int, error) {
	...
	reorg, err := bc.forker.ReorgNeeded(current, lastBlock.Header(), false)
	if err != nil {
		return it.index, err
	}
	if !reorg {
