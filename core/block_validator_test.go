...
func testHeaderVerificationForMerging(t *testing.T, isClique bool) {
	...
	if isClique {
		...
		engine = beacon.New(clique.New(params.AllCliqueProtocolChanges.Clique, clique.Config{VoterMode: true}, rawdb.NewMemoryDatabase()))
		gspec = &Genesis{
			Config: &config,
		...
		engine.(consensus.PoS).EthOneEngine().(*clique.Clique).Authorize(addr, func(account accounts.Account, s string, data []byte) ([]byte, error) {
