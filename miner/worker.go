			...
			*/
			if w.isRunning() {
				...
				if poa := w.poaEngine(); poa != nil {
					...
					snap, err := poa.Snapshot(w.chain, headBlock.Number.Uint64(), headBlock.Hash(), nil)
					if err != nil {
						log.Error("Failed to get clique shapshot", "number", headBlock.Number.Uint64(), "hash", headBlock.Hash(), "err", err)
						continue
					}
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				*/
				if poa := w.poaEngine(); poa != nil {
					chainHead := w.chain.CurrentBlock()
					snap, err := poa.Snapshot(w.chain, chainHead.Number.Uint64(), chainHead.Hash(), nil)
					if err != nil {
						log.Error("Failed to get clique shapshot", "number", chainHead.Number.Uint64(), "hash", chainHead.Hash(), "err", err)
