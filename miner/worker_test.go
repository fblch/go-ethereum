...
func newTestWorkerBackend(t *testing.T, chainConfig *params.ChainConfig, engine consensus.Engine, db ethdb.Database, n int) *testWorkerBackend {
	...
	case *clique.Clique:
		...
		gspec.ExtraData = make([]byte, params.CliqueExtraVanity+common.AddressLength+1+params.CliqueExtraSeal)
		copy(gspec.ExtraData[params.CliqueExtraVanity:params.CliqueExtraVanity+common.AddressLength], testBankAddress.Bytes())
...
func testGenerateBlockAndImport(t *testing.T, isClique bool) {
	...
	if isClique {
		...
		chainConfig.Clique = []params.CliqueConfigEntry{
			{
				Period: 1,
				// MODIFIED by Jakub Pajek (clique params)
				Epoch: params.CliqueEpoch,
