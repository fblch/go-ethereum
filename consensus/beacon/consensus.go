...
func (beacon *Beacon) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header) error {
	...
	// How does the below check affect zero size extra data breaking change?
...
func (beacon *Beacon) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, withdrawals []*types.Withdrawal, dummy bool) (*types.Block, error) {
	if !beacon.IsPoSHeader(header) {
		// MODIFIED by Jakub Pajek (clique static block rewards)
		//return beacon.ethone.FinalizeAndAssemble(chain, header, state, txs, uncles, receipts, nil)
		return beacon.ethone.FinalizeAndAssemble(chain, header, state, txs, uncles, receipts, nil, dummy)
	}
	shanghai := chain.Config().IsShanghai(header.Time)
	if shanghai {
	...
	beacon.Finalize(chain, header, state, txs, uncles, withdrawals, dummy)
