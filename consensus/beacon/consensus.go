...
func (beacon *Beacon) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, dummy bool) (*types.Block, error) {
	...
	if !beacon.IsPoSHeader(header) {
		...
		return beacon.ethone.FinalizeAndAssemble(chain, header, state, txs, uncles, receipts, dummy)
	}
	// Finalize and assemble the block
	// MODIFIED by Jakub Pajek (clique static block rewards)
	//beacon.Finalize(chain, header, state, txs, uncles)
	beacon.Finalize(chain, header, state, txs, uncles, dummy)
