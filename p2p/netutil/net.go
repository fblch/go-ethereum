...
var special4, special6, mobileLan4, mobileLan6 Netlist
...
func IsMobileLAN(ip net.IP) bool {
	return AddrIsMobileLAN(IPToAddr(ip))
}

// ADDED by Jakub Pajek (mobile connectivity)
// AddrIsMobileLAN reports whether an IP is a local mobile network address.
func AddrIsMobileLAN(ip netip.Addr) bool {
	if ip.Is4In6() {
		ip = netip.AddrFrom4(ip.As4())
	}
	if ip.Is4() {
		return mobileLan4.ContainsAddr(ip)
	}
	return mobileLan6.ContainsAddr(ip)
}
...
func CheckRelayAddr(sender, addr netip.Addr) error {
	...
	if !AddrIsMobileLAN(addr) && (AddrIsLAN(addr) && !AddrIsLAN(sender)) {
		return errLAN
	}
	return nil
}
