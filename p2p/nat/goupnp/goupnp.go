...
import (
	...
	"github.com/ethereum/go-ethereum/p2p/nat/goupnp/ssdp"
)
...
func DiscoverDevices(localIPv4MCastAddr net.IP, searchTarget string) ([]MaybeRootDevice, error) {
	// MODIFIED by Jakub Pajek (x/mobile: Calling net.Interfaces() fails on Android SDK 30+)
	//hc, hcCleanup, err := httpuClient()
	hc, hcCleanup, err := httpuClient(localIPv4MCastAddr)
	if err != nil {
		return nil, err
	}
	defer hcCleanup()
