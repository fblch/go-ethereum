...
func (v byteSlice) Size() int {
	return len(v)
}

func TestSizeCountConstrainedCache(t *testing.T) {
	lru := NewSizeCountConstrainedCache[testKey, byteSlice](100, 12)
	var want uint64
...
func TestSizeCountConstrainedCacheSizeOverflow(t *testing.T) {
	lru := NewSizeCountConstrainedCache[testKey, byteSlice](100, 12)
...
// This test adds inserting an element exceeding the max count.
func TestSizeCountConstrainedCacheCountOverflow(t *testing.T) {
	lru := NewSizeCountConstrainedCache[testKey, byteSlice](300, 10)
	...
	// Add one more elem. We expect it to swap out the oldest entry.
	{
		k := mkKey(1337)
		v := make([]byte, 200)
		lru.Add(k, v)
	}
	// Elem 0 should be missing
	{
		k := mkKey(0)
		if _, ok := lru.Get(k); ok {
			t.Fatalf("should be evicted: %v", k)
	...
	// Elems 1-9 should be present
	for i := 1; i < 10; i++ {
		k := mkKey(i)
		if _, ok := lru.Get(k); !ok {
			t.Fatalf("should be present: %v", k)
		}
	}
	// Elem 1337 should be present
	{
		k := mkKey(1337)
		if _, ok := lru.Get(k); !ok {
			t.Fatalf("should be present: %v", k)
	...
	// The size should be accurate
	if have, want := lru.size, uint64(290); have != want {
		t.Fatalf("size wrong, have %d want %d", have, want)
	}
	// Adding one item within the size constraints should swap out
	// the oldest one due to the item count constraints
	{
		i := 0
		k := mkKey(i)
		v := fmt.Sprintf("value-%014d", i)
		lru.Add(k, []byte(v))
		if have, want := lru.size, uint64(300); have != want {
			t.Fatalf("size wrong, have %d want %d", have, want)
		}
	}
}

// This test adds inserting an element exceeding the max size and count
func TestSizeCountConstrainedCacheSizeCountOverflow(t *testing.T) {
	lru := NewSizeCountConstrainedCache[testKey, byteSlice](100, 10)
...
func TestSizeCountConstrainedCacheSameItem(t *testing.T) {
	lru := NewSizeCountConstrainedCache[testKey, byteSlice](100, 11)
...
func TestSizeCountConstrainedCacheEmpties(t *testing.T) {
	lru := NewSizeCountConstrainedCache[testKey, byteSlice](100, 20)
